...
export default function NewRoutePage() {
    ...
    useEffect(() => {setRouteToAdd({...routeToAdd,locations: markers})},[markers] )
    ...
    return (
        <>
            ...
            <div>{markers.map(marker => <Fragment key={`${marker.lat} - ${marker.lng}`}><p>{marker.locationName}</p></Fragment>)}</div>
        </>
    )
}
